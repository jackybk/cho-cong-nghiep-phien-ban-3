...
class SearchApp extends MallbaseApp
{
      ...
      function doctin()
    {
    	...
    	$doctin_mod =& m('qldangtin');
    	$data = array(
				  'view'     => $view + 1,
			);
				 $doctin_mod =& m('qldangtin');
				  $doctin_mod->edit($id, $data);
				  
    	
    	$noidung = $doctin_mod->find(array(
		        'count'         => true,
		        'conditions'    => 'id = ' . $id,
	           ));
    	$id_user= $this->visitor->get('user_id');
        $this->assign('noidung',  $noidung);
        $this->assign('user',  $id_user);
    	$this->display('noidungtin.index.html');
    	//$doctin_mod.clean_cache();
    }
    function luutin()
    {
    	...
    	$luutin_ok=isset($_GET['luutin']) ? trim($_GET['luutin']) : '';
    	$result='1';
    	if($user_id == 0)
    	{
    		$this->show_message('chuatk');
    	}
    	else {
	    	if($luutin_ok=='1')
	    	{
	    		$this->show_message('luutin_curent');
	    	}
	    	else 
	    	{
	    	$data = array(
				 ...
				 $doctin_mod =& m('qldangtin');
				  $doctin_mod->edit($id, $data);
				 if ( $doctin_mod->has_error())
				  {
				       $this->pop_warning( $doctin_mod->get_error());
				       return;
				  }
				    $this->show_message('luutin_ok',
				    'back_list',    'index.php?app=search&amp;act=doctin&amp;id='.$id
				  );
	    	}
       }
    }

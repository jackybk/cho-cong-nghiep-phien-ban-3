...
$(function(){
    ...
    $('#truncate').click(function(){
        ...
        if(goods_ids && confirm('<?php echo sprintf('Bạn muốn xóa tất cả %s sản phẩm? Hủy bỏ các sản phẩm sẽ không thể được phục hồi!', $this->_var['page_info']['item_count']); ?>')){
...
<div class="content">
  ...
  <div id="right">
    <?php echo $this->fetch('member.submenu.html'); ?>
        <div class="wrap">
            <div class="eject_btn_two eject_pos1" title="Nhập Taobao"><b class="ico1" onclick="go('index.php?app=my_goods&amp;act=import_taobao');">Nhập Taobao</b></div>
            <div class="eject_btn_two eject_pos2" title="Thêm vào"><b class="ico2" onclick="go('index.php?app=my_goods&amp;act=add');">Thêm vào</b></div>
                ...
                <table id="my_goods" server="<?php echo $this->_var['site_url']; ?>/index.php?app=my_goods&amp;act=ajax_col" >
                    ...
                    <tr class="line_bold">
                        ...
                        <th class="align1" colspan="2">
                            ...
                            <a href="javascript:void(0);" class="edit" ectype="batchbutton" uri="index.php?app=my_goods&act=batch_edit" name="id">Sửa</a>
                            <a href="javascript:void(0);" class="delete" ectype="batchbutton" uri="index.php?app=my_goods&act=drop" name="id" presubmit="confirm('Bạn có chắc chắn muốn xóa nó ?')">Xoá</a>
                        </th>
                        <th colspan="8">
                            <div class="select_div">
                            <form id="my_goods_form" method="get">
                            <a id="truncate" class="detlink" style="float:right" href="javascript:;">Xóa tất cả</a>
                            <?php if ($this->_var['filtered']): ?>
                            <a class="detlink" style="float:right" href="<?php echo url('app=my_goods'); ?>">Hủy tìm kiếm</a>
                            <?php endif; ?>
                            <input type="hidden" name="app" value="my_goods">
                            <select class="select1" name='sgcate_id'>
                                <option value="0">Danh mục cửa hàng</option>
                    ...
                    <tr class="gray"  ectype="table_header">
                        <th width="30"></th>
                        <th width="55"></th>
                        <th width="165" coltype="editable" column="goods_name" checker="check_required" inputwidth="90%" title="Sắp xếp theo"  class="cursor_pointer"><span ectype="order_by">Tên sản phẩm</span></th>
                        <th width="70" column="cate_id" title="Sắp xếp theo"  class="cursor_pointer"><span ectype="order_by">Danh mục sản phẩm</span></th>
                        <th width="55" coltype="editable" column="brand" checker="check_required" inputwidth="55px" title="Sắp xếp theo"  class="cursor_pointer"><span ectype="order_by">Hãng sản xuất</span></th>
                        <th width="55" class="cursor_pointer" coltype="editable" column="price" checker="check_number" inputwidth="50px" title="Sắp xếp theo"><span ectype="order_by">Giá</span></th>
                        <th width="55" class="cursor_pointer" coltype="editable" column="stock" checker="check_pint" inputwidth="50px" title="Sắp xếp theo"><span ectype="order_by">Số lượng</span></th>
                        <th width="25" coltype="switchable" column="if_show" onclass="right_ico" offclass="wrong_ico" title="Sắp xếp theo"  class="cursor_pointer"><span ectype="order_by">Hiển thị</span></th>
                        <th width="25" coltype="switchable" column="recommended" onclass="right_ico" offclass="wrong_ico" title="Sắp xếp theo"  class="cursor_pointer"><span ectype="order_by">Giới thiệu</span></th>
                        <th width="25" column="closed" title="Sắp xếp theo" class="cursor_pointer"><span ectype="order_by">Đóng</span></th>
                        <th>Hành động</th>
                    </tr>
                    <?php endif; ?>
...
?>
                    <tr class="line<?php if (($this->_foreach['_goods_f']['iteration'] == $this->_foreach['_goods_f']['total'])): ?> last_line<?php endif; ?>" ectype="table_item" idvalue="<?php echo $this->_var['goods']['goods_id']; ?>">
                        ...
                        <td><div><a href="<?php echo url('app=my_goods&act=edit&id=' . $this->_var['goods']['goods_id']. ''); ?>" class="edit">Sửa</a>
                            <a  href="javascirpt:;" ectype="dialog" dialog_id="export_ubbcode" dialog_title="Mã BLOG" dialog_width="380" uri="<?php echo url('app=my_goods&act=export_ubbcode&id=' . $this->_var['goods']['goods_id']. ''); ?>" class="export">Mã BLOG</a> <a href="javascript:drop_confirm('Bạn có chắc chắn muốn xóa nó ?', 'index.php?app=my_goods&amp;act=drop&id=<?php echo $this->_var['goods']['goods_id']; ?>');" class="delete">Xoá</a></div></td>
                    </tr>
                    <?php endforeach; else: ?>
                    <tr>
                        <td class="align2 member_no_records padding6" colspan="10"><?php echo $this->_var['lang'][$_GET['act']]; ?>Không có hồ sơ</td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <th colspan="10">
                            <p class="position1">
                                ...
                                <a href="javascript:void(0);" class="edit" ectype="batchbutton" uri="index.php?app=my_goods&amp;act=batch_edit&ret_page=<?php echo $this->_var['page_info']['curr_page']; ?>" name="id">Sửa</a>
                                <a href="javascript:void(0);" class="delete" uri="index.php?app=my_goods&act=drop" name="id" presubmit="confirm('Bạn có chắc chắn muốn xóa nó ?')" ectype="batchbutton">Xoá</a>
                            </p>
                            <p class="position2">

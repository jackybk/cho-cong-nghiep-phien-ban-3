...
function use_theme(template, style){
    ...
    {
        if (!confirm('Template không phù hợp!! Khuyến cáo nên dừng lại'))
        {
            return;
        }
...
<div id="rightTop">
<p>
    <b>Template tham khảo</b><br />
Đang sử dụng:<?php echo $this->_var['curr_template_name']; ?>&nbsp;<?php echo $this->_var['curr_style_name']; ?>
</p>
...
?>
    <li>
        ...
        <input type="button" value="Sử dụng" onclick="use_theme('<?php echo $this->_var['template_name']; ?>', '<?php echo $this->_var['style_name']; ?>');" class="formbtn" />&nbsp;&nbsp;
        <input type="button" value="Xem trước" onclick="preview_theme('<?php echo $this->_var['template_name']; ?>', '<?php echo $this->_var['style_name']; ?>');" class="formbtn" />
        <?php else: ?>
        <input type="button" value="Xem" onclick="go_index()" class="formbtn" />
        <?php endif; ?>
        </div>
    </li>
